...
class PlankPositionDetector:
    ...
    def check(self, pose_landmarks, avg_torso):
        ...
        #fourth check: we want to ensure that the object is in a "9 o'clock" position, meaning the shoulders are to the left of the ankles
        is_9_oclock = shoulder_x < ankle_x

        self.last_debug_msg = f"Vis: {left_visible or right_visible} | Horiz: {is_horizontal} (dx:{delta_x:.1f}, dy:{delta_y:.1f}) | W_Low: {wrists_are_lowest} | A_Low: {ankles_are_lowest} | 9_O'Clock: {is_9_oclock}, shoulder_x: {shoulder_x:.3f}, ankle_x: {ankle_x:.3f}"
        is_valid = is_horizontal and is_grounded and is_9_oclock
...
def process_frame(frame, rep_sm, hip_sm, cache, timestamp_ms, is_live=False, start_time=0, process_this_frame=True, frame_num=0, plank_detector=None, workout_stats=None):
    ...
    if plank_detector and plank_detector.is_in_plank:
        box_color = (0, 255, 0) # ירוק - ה-Handshake עבר בהצלחה!
    else:
        box_color = (0, 165, 255) # כתום - ממתין שהמשתמש ייכנס למסגרת
        
    # ציור המסגרת על גבי הוידאו
