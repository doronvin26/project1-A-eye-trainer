...
def get_default_labels_for_frame(frame_idx):
    ...
    Looks backwards to find the closest previously labeled frame.
    """
    if frame_idx == 0:
        return {"is_valid_frame": True, "pushup_phase": "High", "hips_position": "Good"}
    
    # Look backwards to find the most recently labeled frame
    for i in range(frame_idx - 1, -1, -1):
        if i in st.session_state.frame_labels:
            prev_labels = st.session_state.frame_labels[i]
            return {
    ...
    # Fall back to standard defaults if no previous labels found
    return {"is_valid_frame": True, "pushup_phase": "High", "hips_position": "Good"}
...
if st.session_state.video_path is not None:
    ...
    with col_labels:
        ...
        # 1. Initialize dictionary state for this frame if missing
        ...
        current_labels = st.session_state.frame_labels[st.session_state.current_frame_idx]
        
        # 2. Pre-sync Streamlit Widget state keys
        checkbox_key = f"bad_frame_{st.session_state.current_frame_idx}"
        if checkbox_key not in st.session_state:
            st.session_state[checkbox_key] = not current_labels.get("is_valid_frame", True)
            
        pushup_key = f"pushup_{st.session_state.current_frame_idx}"
        if pushup_key not in st.session_state:
            st.session_state[pushup_key] = current_labels.get("pushup_phase", "High")
            
        hips_key = f"hips_{st.session_state.current_frame_idx}"
        if hips_key not in st.session_state:
            st.session_state[hips_key] = current_labels.get("hips_position", "Good")

        # 3. Render Checkbox using the key
        ...
        # 4. Render Radios if frame is valid
        if not bad_frame:
